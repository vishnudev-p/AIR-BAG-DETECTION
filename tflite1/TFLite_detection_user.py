...
def create_signup_page():
    global user_entry, user_pass, user_email  # Declare global variables
    ...
    # Email (dummy section)
    user_email = ctk.CTkEntry(app, placeholder_text="Email (optional)")
...
def create_welcome_page():
    ...
    button_ready = ctk.CTkButton(app, text='READY FOR SAFE DRIVE', command=start_detection)
    button_ready.pack(pady=20)
...
def start_detection():
    ...
    # Open video file
    video = cv2.VideoCapture('C:\\Users\\HP\\Desktop\\AIR BAG DETECTION\\tflite1\\airbag1.mp4')
    ...
    while(video.isOpened()):
        ...
        for i in range(len(scores)):
            if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
                ...
                # Check if airbag is detected
                if object_name == "airbag":
...
def send_email():
    ...
    msg['Subject'] = "Emergency: Airbag Detected! Accident Occured"
    msg['From'] = sender_email
    msg['To'] = ", ".join(receiver_emails)

    # Create the body of the message (a plain-text and an HTML version)
    text = "Airbag has been detected. There's been an accident, and the person is in danger."
    html = """\
    <html>
      <body>
        <p>Airbag has been detected. There's been an accident, and the person is in danger. Please act immediately.</p>
      </body>
    </html>
    """
